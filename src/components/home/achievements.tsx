      ...
      <div className="space-y-6 pt-4 overflow-hidden">
        ...
        <Marquee pauseOnHover repeat={4} reverse={false} className="[--duration:28s] py-3">
          {data.certificates.map((cert) => (
            <div
              key={cert.title}
              className="w-72 sm:w-80 shrink-0 group relative flex flex-col justify-between overflow-hidden rounded-2xl border border-border/60 bg-background/70 backdrop-blur-md transition-all duration-500 hover:border-indigo-500/80 hover:shadow-2xl hover:shadow-indigo-500/20 hover:scale-[1.03]"
            >
              {/* Certificate Preview Image */}
              {cert.image && (
                <div
                  className="relative w-full h-44 overflow-hidden border-b border-border/40 bg-zinc-950/90 cursor-pointer"
                  onClick={() =>
                    handleOpenPhoto({
                      src: cert.image,
                ...
                >
                  ...
                  {/* Uncropped Certificate Image */}
                  <Image
                    src={cert.image}
                    alt={`${cert.title} Certificate`}
                    fill
                    className="object-contain p-1.5 transition-transform duration-500 group-hover:scale-105 z-10"
                    sizes="320px"
                  />

                  {/* Gradient Overlay & Zoom Badge */}
                  <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-80 group-hover:opacity-40 transition-opacity duration-300 z-20" />

                  <div className="absolute top-3 right-3 z-30 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-black/80 text-indigo-400 text-xs font-medium backdrop-blur-md border border-indigo-500/40">
                      <IconMaximize className="h-3.5 w-3.5" />
                      <span>View</span>
                    </span>
                  </div>
                  ...
                  <div className="absolute bottom-3 left-3 right-3 z-30 flex items-center justify-between text-white">
                    <span className="text-xs font-semibold px-2 py-0.5 rounded bg-indigo-600/90 backdrop-blur-sm text-white">
                      {cert.issuer}
                    </span>
                    <span className="text-xs font-medium px-2 py-0.5 rounded bg-black/80 backdrop-blur-sm text-zinc-200 tabular-nums">
                      {cert.date}
                    </span>
                  </div>
                </div>
              ...
              <div className="p-4 flex flex-col justify-between flex-1 space-y-3">
                <div className="space-y-2">
                  <h4 className="text-sm font-bold tracking-tight text-foreground group-hover:text-indigo-400 transition-colors line-clamp-1">
                    {cert.title}
                  </h4>
                  <div className="flex flex-wrap gap-1 pt-0.5">
                    {cert.skills.map((skill) => (
                      <Badge
                        key={skill}
                        variant="secondary"
                        className="text-[10px] px-1.5 py-0.5 bg-muted/80 hover:bg-indigo-500/10 hover:text-indigo-400 transition-colors"
                      >
                        {skill}
                      </Badge>
                    ))}
                ...
                <div className="pt-2 border-t border-border/40 flex items-center justify-between">
                  <button
                    ...
                    className="inline-flex items-center gap-1 text-xs font-medium text-muted-foreground hover:text-indigo-400 transition-colors"
                  >
                    <IconMaximize className="h-3.5 w-3.5" />
                    <span>Full View</span>
                  </button>
